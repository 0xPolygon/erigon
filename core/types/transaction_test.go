...
func TestStateSyncTx_HashSensitivity(t *testing.T) {
	...
	hashOf := func(mutate func(*StateSyncTx)) libcommon.Hash {
		// deep copy
	...
	cases := []struct {
		...
		{"change Contract", func(st *StateSyncTx) {
			st.StateSyncData[0].Contract = libcommon.HexToAddress("0x000000000000000000000000000000000000CAFE")
		}},
		{"change Data", func(st *StateSyncTx) {
			st.StateSyncData[0].Data = []byte("alpha-mutated")
		}},
		{"change TxHash", func(st *StateSyncTx) {
			st.StateSyncData[0].TxHash = libcommon.HexToHash("0xAAAA")
		}},
		{"mutate second entry", func(st *StateSyncTx) {
			st.StateSyncData[1].Contract = libcommon.HexToAddress("0x000000000000000000000000000000000000BEEF")
			st.StateSyncData[1].Data = []byte("beta-mutated")
			st.StateSyncData[1].TxHash = libcommon.HexToHash("0xBBBB")
		}},
		{"append new entry contiguous ID", func(st *StateSyncTx) {
			st.StateSyncData = append(st.StateSyncData, &StateSyncData{
				...
				Contract: libcommon.HexToAddress("0x0000000000000000000000000000000000001003"),
				Data:     []byte("gamma"),
				TxHash:   libcommon.HexToHash("0x3333"),
			})
		}},
	}
...
func makeBaseStateSyncTx() *StateSyncTx {
	return &StateSyncTx{
		StateSyncData: []*StateSyncData{
			{
				ID:       1,
				Contract: libcommon.HexToAddress("0x0000000000000000000000000000000000001001"),
				Data:     []byte("alpha"),
				TxHash:   libcommon.HexToHash("0x1111"),
			},
			{
				ID:       2,
				Contract: libcommon.HexToAddress("0x0000000000000000000000000000000000001002"),
				Data:     []byte("beta"),
				TxHash:   libcommon.HexToHash("0x2222"),
			},
		},
	}
}
