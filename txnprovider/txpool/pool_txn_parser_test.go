		...
		t.Run(strconv.Itoa(int(testSet.chainID.Uint64())), func(t *testing.T) {
			...
			for i, tt := range testSet.tests {
				...
				t.Run(strconv.Itoa(i), func(t *testing.T) {
					...
					require.Equal(tt.Nonce, txn.Nonce)
				})
			}
		})
	}
...
func TestTxnSlotsGrowth(t *testing.T) {
	assert := assert.New(t)
	s := &TxnSlots{}
	s.Resize(11)
...
func TestSetCodeTxnParsing(t *testing.T) {
	...
	var txn TxnSlot
	txnType, err := PeekTransactionType(bodyRlx)
	require.NoError(t, err)
...
func TestSetCodeTxnParsingWithLargeAuthorizationValues(t *testing.T) {
	// generated using this in core/types/encdec_test.go
	/*
		func TestGenerateSetCodeTxnRlpWithLargeAuthorizationValues(t *testing.T) {
			tr := NewTRand()
			var txn. Transaction
			requiredAuthLen := 1
			for txn. = tr.RandTransaction(); txn..Type() != types.SetCodeTxnType || len(txn..(*SetCodeTransaction).GetAuthorizations()) != requiredAuthLen; txn. = tr.RandTransaction() {
...
var txnParseDevNetTests = []parseTxnTest{
...
var txnRopstenTests = []parseTxnTest{
...
var allNetsTestCases = []struct {
...
}{
	{
		...
		tests:   TxnParseMainnetTests,
	},
	{
		chainID: *uint256.NewInt(123),
		tests:   txnParseCalaverasTests,
	},
	{
		chainID: *uint256.NewInt(1337),
		tests:   txnParseDevNetTests,
	},
	{
		chainID: *uint256.NewInt(3),
		tests:   txnRopstenTests,
	},
}

var TxnParseMainnetTests = []parseTxnTest{
	// Legacy unprotected
